...
if(0):
  ...
  xdmf_file = XDMFFile(MPI.comm_world, mesh_folder+"single_crystal_notched_bar.xdmf")
  xdmf_file.write(mesh)
  #mesh_file = File(mesh_folder+"single_crystal_notched_bar.xml")
  #mesh_file << mesh

# Load mesh
mesh_path = mesh_folder+"single_crystal_notched_bar"
...
class Grain1(SubDomain):
    def inside(self, x, on_boundary):
        return ((x[0]>e-.02) and ((x[0]+delta*(x[1]-W/2))<(((L)/2.))) and (x[1]>(e-0.02)) and (x[1]<(W-e+0.02)))
class Grain2(SubDomain):
    def inside(self, x, on_boundary):
        return (((x[0]+delta*(x[1]-W/2))>((L/2.)-0.02) and (x[0]<(L)) and (x[1]>e-0.02) and (x[1]<(W-e)+0.02)))
class Box(SubDomain):
    def inside(self, x, on_boundary):
...
save_folder = "SurfingBCNotched/" # The path must exist
